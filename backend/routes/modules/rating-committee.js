  ...
  fastify.register((instance, opts, done) => {
    ...
    fastify.post("/rating_committee_meetings", async (request, reply) => {
      try {
        ...
        if (mhm.length > 0 && request.active_role_name === "Committee Member") {
          whereClause["id"] = mhm;
        }
    ...
    fastify.post(
      ...
      async (request, reply) => {
        try {
          ...
          const rating_committee_meeting_register = await DB_CLIENT.query(
      ...
      try {
        ...
        params =  await Promise.all(params.map( async el=> {
          ...
          if(previous_record.length){
            params['previous_rating'] = previous_record[0].long_term_rating_assgined_text;
            params['previous_outlook'] = previous_record[0].long_term_outlook;
          }
    ...
    fastify.post(
      ...
      async (request, reply) => {
        try {
          ...
          await DB_CLIENT.query(
            `SELECT DISTINCT c.uuid, c.name, m.mandate_id, m.mandate_type, m.total_size, rcmr.voting_status
      ...
      try {
        ...
        switch(request.active_role_name) {
          ...
          rating_register = await DB_CLIENT.query(
            `SELECT null AS model_rating, rcmr.uuid, rcmr.instrument_text, rcmr.instrument_size_number,
    ...
    fastify.post(
      ...
      async (request, reply) => {
        try {
          ...
          console.log("final_rating: ", final_rating[0].voting_score);
          ...
          if((final_rating[0].voting_score >= Math.ceil(meeting_member[0].cnt/2)) || (voting_count.length > meeting_member[0].cnt - 1))
            {
            ...
            await DB_CLIENT.query(
              `UPDATE rating_committee_meeting_registers set long_term_rating_assgined_text = :long_term_rating_assgined_text, long_term_outlook =  :long_term_outlook
...
`,
    ...
    fastify.post("/voting/status", async (request, reply) => {
      try {
        ...
        let voting_status = await DB_CLIENT.query(
          `
        SELECT DISTINCT rcmr.instrument_detail_id
        AS instrument_detail_id, '' AS dissent_remark, null AS dissent, null AS rating_team_non_cooperation, rcm.meeting_at, c.name AS company_name, c.uuid AS company_uuid, m.mandate_id AS gen_mandate_id, i.name AS instrument_name, rcvm.rating AS members_rating, rcvm.outlook AS members_outlook, rcmr.mandate_id AS mandate_id, u.full_name AS group_head, rcmr.long_term_rating_recommendation AS rating_team_recommendation, rcvm.rating AS rc_members_output, rcmr.remark AS remarks, rcmr.voting_status AS voting_status from rating_committee_meeting_registers rcmr 
        INNER JOIN rating_committee_voting_metadata rcvm ON rcvm.instrument_detail_id  = rcmr.instrument_detail_id 
