...
async function inbox_routes(fastify) {
  fastify.post("/inbox/execution/assign_to_user", async (request, reply) => {
    try {
      ...
      const result = mandates.map(async (el) => {
        ...
        if (!workflow_instance || !rating_process) {
  ...
  fastify.post("/inbox/execution/upload_doc", async (request, reply) => {
    try {
      ...
      if (!workflow_instance || !rating_process || !company) {
        (reply.status_code = 422),
          reply.send({
            success: false,
            error: "No Workflow Found",
...
rating_process_id: rating_process.id,
        ...
        workflow_document_remark || workflow_document_remark.setWorkflow_document(workflow_document);
  ...
  fastify.post("/inbox/execution/download_doc", async (request, reply) => {
    try {
      ...
      const workflow_doc = await DB_CLIENT.query(
        ...
        WHERE wd.company_id =:company_id ORDER BY wd.updated_at DESC
        `,
        {
          replacements: {
            company_id: company.id,
  ...
  fastify.post("/inbox/view_executables", async (request, reply) => {
    try {
      ...
      var companies = await DB_CLIENT.query(
       ...
       where a.code = :code AND wil.is_active =1 AND rp.id= :rating_process_id AND c.id = :company_id AND wil.performed_by = :performed_by ) AS my_query ORDER BY mandate_id DESC`,
        {
          replacements: {
            company_id: company.id,
            code: params["code"],
      ...
      if (companies[0].is_last_activity) {
        last_activity_record = await DB_CLIENT.query(
          `SELECT DISTINCT rcvm.instrument_detail_id, m.mandate_id, CONCAT(rcmr.sub_category_text, '/', rcmr.category_text, '/', rcmr.instrument_text  ) AS instrument, rcvm.rating, rcvm.outlook, sbq.created_at AS press_release_date FROM rating_committee_meeting_registers rcmr 
          ...
          LEFT JOIN ( SELECT TOP 1 wd.created_at,wd.mandate_id FROM workflow_documents wd INNER JOIN mandates m2 ON m2.id = wd.mandate_id  INNER JOIN
          companies c2 ON c2.id = m2.company_id WHERE c2.id =:company_id AND wd.created_at IS NOT NULL AND wd.rating_process_id = :rating_process_id
          ORDER BY wd.created_at DESC ) AS sbq
          ON sbq.mandate_id = m.id
          WHERE a.code = :code AND wil.performed_by = :performed_by AND rp.id= :rating_process_id AND c.id = :company_id 
          `,
          {
            replacements: {
              company_id: company.id,
  ...
  fastify.post("/inbox", async (request, reply) => {
    try {
      ...
      let companies = await DB_CLIENT.query(
        ...
        a.code, a.name AS activity_to_be_performed, c.name AS company_name,c.uuid AS company_uuid,
        rp.name AS rating_process_name, rp.uuid AS rating_process_uuid from companies c INNER JOIN mandates m ON m.company_id = c.id 
                       INNER JOIN workflow_instances wi ON wi.mandate_id = m.id
                       INNER JOIN workflow_instances_log wil ON wil.workflow_instance_id  = wi.id
                       ...
                       GROUP BY a.name,a.code, c.name,rp.name,c.uuid,rp.uuid,wl_sq.from_user,wl_sq.from_user_code,wc.tat,wil.created_at ORDER BY wil.created_at DESC        
      `,
        {
          replacements: {
            id: request.user.id,
