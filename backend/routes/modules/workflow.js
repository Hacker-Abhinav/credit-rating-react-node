      ...
      try {
        ...
        const company_mandates = await DB_CLIENT.query(
          ...
          LEFT JOIN (SELECT u.full_name AS bd_name, u.id, u.uuid AS bd_uuid from users u LEFT JOIN mandates m2 ON m2.bd_id =u.id ) AS sbq2 ON sbq2.id = m.bd_id
           where  wil.is_active =1 AND c.id = :company_id  AND m.is_active = 1 ) AS my_query ORDER BY created_at DESC
        `,
          {
            replacements: {
    ...
    fastify.post("/mandate_lifecycle/view", async (request, reply) => {
      try {
        ...
        let mandate_status = await DB_CLIENT.query(
          `
        SELECT a.code, a.name, rating_process, performer_role, assigner_role, performed_by_user, wil.created_at, wil.updated_at FROM workflow_instances_log wil 
...
INNER JOIN users u ON u.id = m.ra_id 
 ...
 AND ti.is_active = 1 AND m.is_active = 1
      ...
      async (request, reply) => {
        try {
          ...
          params =  await Promise.all(params.map( async el=> {
            ...
            console.log("mandate_res, tra_res ", tra_res);
          ...
          const mandate_ids = params.filter(el=> el.mandate_id);

        const workflow_instance = mandate_ids.map(async (el)=> {
 ...
 ORDER BY m.updated_at DESC
    ...
    fastify.post(
      ...
      async (request, reply) => {
        ...
        WHERE u.id IN ( SELECT urt.report_to_user_id  FROM user_reports_to urt WHERE urt.user_id =:user_id )
