    ...
    async (request, reply) => {
      try {
        ...
        if (error.name === 'SequelizeUniqueConstraintError') {
          ...
          reply.send({ success: false, error: "This director already exists"});
      } else {
        reply.statusCode = 422;
  ...
  fastify.post("/companies/edit_mandates", async (request, reply) => {
    try {
      ...
      const last_updated_mandate = await DB_CLIENT.query(
        `SELECT id from mandates order by created_at DESC `,
        {
          type: QueryTypes.SELECT,
        }
      );
    ...
    async (request, reply) => {
      try {
        ...
        const instrument_detail = await InstrumentDetail.create({
          ...
          updated_at: new Date(),
          created_by: request.user.id,
          financial_year_id: financial_year_id[0].id,
          rating_process_id: workflow_instance.length ? workflow_instance[0][0].rating_process_id : null
        });
  ...
  fastify.post(
    ...
    async (request, reply) => {
      try {
        ...
        const rating_metadata = await DB_CLIENT.query(
          `SELECT id.uuid AS instrument_details_uuid,rp.name AS rating_cycle, rcmr.long_term_outlook  AS outlook,
          rcmr.long_term_rating_assgined_text AS rating, id.press_release_date  AS press_release, id.rating_acceptance_status  
                    AS rating_acceptance, ti.uuid AS transaction_instrument_uuid, rcm.meeting_at AS meeting_date, id.instrument_size  
         ...
         WHERE ti.id = :transaction_instrument_id`,
          {
            replacements: {
              transaction_instrument_id: transaction_instrument.id,
            },
