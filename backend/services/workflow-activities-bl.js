...
const workflow_progress_track = async (params) => {
  ...
  try {
    console.log("await workflow_progress_track: ", params.instance);

    await DB_CLIENT.query(
      `UPDATE workflow_instances_log set is_active = 0, updated_at= :updated_at, updated_by= :updated_by WHERE workflow_config_id IN (SELECT id FROM workflow_configs WHERE current_activity_id IN ( SELECT wc3.next_activity_id FROM workflow_configs wc3 WHERE wc3.current_activity_id IN
...
const ACTIVITY_LOGICS = async (params) => {
  return new Promise( async (resolve,reject) =>{
    ...
    switch (params.activity_code) {
      ...
      mandates m ON m.id = ti.mandate_id Where m.id = :mandate_id) `,
        {
          replacements: {
            mandate_id: params.mandate_id,
            provisional_communication_date: params.request.body.provisional_communication_date ? params.request.body.provisional_communication_date.value : moment().format('YYYY-MM-DD HH:mm:ss'),
          },
          type: QueryTypes.UPDATE,
        }
      );
      ...
      case "10700":
        console.log("params.request.body.acceptance_date.value: ", params.request.body.acceptance_date.value);
        console.log("params.request.body.acceptance_status.value: ", params.request.body.acceptance_status.value);
        ...
        mandates m ON m.id = ti.mandate_id Where m.id = :mandate_id) `,
          {
            replacements: {
              mandate_id: params.mandate_id,
              acceptance_date: params.request.body.acceptance_date ? params.request.body.acceptance_date.value : moment().format('YYYY-MM-DD HH:mm:ss'),
              acceptance_status: params.request.body.acceptance_status ? params.request.body.acceptance_status.value : moment().format('YYYY-MM-DD HH:mm:ss'),
            },
            type: QueryTypes.UPDATE,
          }
        );
      ...
      case "10800":
        ...
        mandates m ON m.id = ti.mandate_id Where m.id = :mandate_id) `,
        {
          replacements: {
            mandate_id: params.mandate_id,
            rating_letter_date: params.request.body.rating_letter_date ? params.request.body.rating_letter_date.value : null
          },
          type: QueryTypes.UPDATE,
        }
       );
      ...
      up_res = await DB_CLIENT.query(
        ...
        mandates m ON m.id = ti.mandate_id Where m.id = :mandate_id) `,
        {
          replacements: {
            mandate_id: params.mandate_id,
            press_release_date: params.request.body.press_release_date ? params.request.body.press_release_date.value : moment().format('YYYY-MM-DD HH:mm:ss'),
          },
          type: QueryTypes.UPDATE,
        }
       );
